...
class EmailIngestionEngine:
    ...
    def parse_email(self):
        ...
        if self.api_key:
            self.forensic_data["ai_analysis"] = self._analyze_threat_with_ai(self.forensic_data["body_artifacts"]["raw_body"])
        else:
            self.forensic_data["ai_analysis"] = {
                "score": "N/A", 
                "analysis": "API Key not provided. AI offline.",
                "mitigations": ["Configure email gateway.", "Block unverified senders."]
            }
...
"""
            ...
            return {"score": score, "analysis": analysis, "mitigations": mitigations}
        except Exception as e:
            return {"score": "Error", "analysis": str(e), "mitigations": "Error generating mitigations."}

    def _get_geolocation(self, ip):
        if ip == "Hidden/Unknown" or ip.startswith("10.") or ip.startswith("192.168."):
            ip = "8.8.8.8" 
        try:
        ...
        return {"country": "Unknown", "city": "Unknown", "lat": 0.0, "lon": 0.0, "isp": "Unknown"}
    ...
    def _extract_body_artifacts(self):
        ...
        return {"extracted_urls": list(set(re.findall(url_pattern, body))), "raw_body": body}
